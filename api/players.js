...
function rating(p,v){
  const age=ageAt(p.date_of_birth);
  const base=Math.log10((+v||0)+1)*10;
...
module.exports = async (req,res)=>{
  try{
    ...
    let base=playersCache.filter(p=>
      allowed.has(String(p.current_club_domestic_competition_id)) &&
      (league==='ALL'||String(p.current_club_domestic_competition_id)===league) &&
      (!q||String(p.name||'').toLowerCase().includes(q)) &&
      (pos==='all'||String(p.position||'')===pos) &&
      ageAt(p.date_of_birth)<=maxAge
    );
    ...
    const results=base.map(p=>{
      ...
      return {
        ...
        name:p.name,
        position:p.position,
        age:ageAt(p.date_of_birth),
        nationality:p.country_of_citizenship||'',
        foot:p.foot||'',
        value,
        rating:rating(p,value),
    ...
    res.status(502).json({
      error:'Impossible de charger la base publique',
