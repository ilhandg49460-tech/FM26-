...
function renderScoutPlayers(list){
  ...
  box.innerHTML=lastScoutingResults.map((p,i)=>`<div class="player scout-player" data-player-index="${i}" style="cursor:pointer">
    <div><b>${escapeHtml(p.name)}</b><br>
    <small>${escapeHtml(p.position||'—')} · ${p.age} ans · ${escapeHtml(p.nationality||'—')}</small><br>
    <small>${formatMoney(p.value)} · compatibilité ${compatibility(p)}%</small></div>
...
function openPlayer(i){
  ...
  d.innerHTML=`<button class="secondary" id="closePlayer">Fermer</button>
    <h3>${escapeHtml(p.name)}</h3>
    <p>${escapeHtml(p.position||'—')} · ${p.age} ans · ${escapeHtml(p.nationality||'Nationalité inconnue')}</p>
    <p>⭐ <b>${p.rating}/99</b> · 💰 <b>${formatMoney(p.value)}</b> · 🦶 ${escapeHtml(p.foot||'—')}</p>
    <p>🎯 <b>${comp}% compatible</b> avec ton budget club (${c.toFixed(1)} M€).</p>
    <div class="result"><b>Analyse FM26+</b><br>
    ${p.age<=23?'🟢 Profil jeune et intéressant pour progression/revente.':'🟡 Profil plus expérimenté.'}<br>
    ${p.value/1e6<=c?'🟢 Valeur dans le budget de référence.':'🔴 Valeur supérieure au budget de référence.'}</div>`;
  document.getElementById('closePlayer').addEventListener('click',()=>d.style.display='none');
