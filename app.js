...
function renderScoutPlayers(list){
 ...
 box.innerHTML=lastScoutingResults.map((p,i)=>`<div class="player" onclick='openPlayer(${i})' style="cursor:pointer"><div><b>${p.name}</b><br><small>${p.position} · ${p.age} ans · ${p.nationality||'—'}</small><br><small>${formatMoney(p.value)} · compatibilité ${compatibility(p)}%</small></div><div class="rating">${p.rating}</div></div>`).join('')||'<div class="result">Aucun joueur.</div>';
}
function openPlayer(i){
 const p=lastScoutingResults[i]; if(!p)return;
 const c=clubBudget(), comp=compatibility(p);
 const d=document.getElementById('playerDetail');d.style.display='block';
 d.innerHTML=`<button onclick="document.getElementById('playerDetail').style.display='none'">Fermer</button>
 <h3>${p.name}</h3><p>${p.position} · ${p.age} ans · ${p.nationality||'Nationalité inconnue'}</p>
 <p>⭐ <b>${p.rating}/99</b> · 💰 <b>${formatMoney(p.value)}</b> · 🦶 ${p.foot||'—'}</p>
 <p>🎯 <b>${comp}% compatible</b> avec ton budget club (${c.toFixed(1)} M€).</p>
 <div class="result"><b>Analyse FM26+</b><br>${p.age<=23?'🟢 profil jeune et intéressant pour une progression/revente.':'🟡 profil plus expérimenté.'}<br>${p.value/1e6<=c?'🟢 valeur dans le budget de référence.':'🔴 valeur supérieure au budget de référence.'}</div>`;
