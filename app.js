...
async function searchRealDB(){
 ...
 status.innerHTML='<b>⏳ Recherche...</b><br>Le serveur prépare les résultats. Première requête potentiellement plus longue.';
 const q=encodeURIComponent(document.getElementById('realSearch').value||'');
 const league=document.getElementById('realLeague').value;
 const pos=document.getElementById('realPos').value;
 const age=document.getElementById('realAge').value||35;
 const value=document.getElementById('realValue').value||250;
 try{
  const r=await fetch(`/api/players?q=${q}&league=${league}&pos=${pos}&age=${age}&value=${value}`);
  const data=await r.json();
  if(!r.ok)throw Error(data.error||'Erreur serveur');
  status.innerHTML=`<b>✅ ${data.count} résultats</b><br>Base publique · valorisation retenue au plus tard le 01/07/2025.`;
  box.innerHTML=data.players.map(p=>`<div class="player"><div><b>${p.name}</b><br><small>${p.position} · ${p.age} ans · ${p.nationality||'—'} · ${p.foot||'—'}</small><br><small>${formatMoney(p.value)} · données historiques</small></div><div class="rating">${p.rating}</div></div>`).join('')||'<div class="result">Aucun joueur.</div>';
 }catch(e){status.innerHTML='<b>❌ Base indisponible</b><br>'+e.message+'<br><small>La recherche ne fonctionne que lorsque le projet Vercel a bien déployé le dossier /api.</small>'}
}
