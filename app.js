...
const REAL_BASE='https://huggingface.co/datasets/ngeorgea/transfermarkt-player-scores/resolve/main/';
const REAL_LEAGUES=new Set(['GB1','ES1','IT1','L1','FR1']);
let realPlayers=[], realVals=[], realLoaded=false;
function csvParse(text){
 const rows=[];let row=[],cell='',quoted=false;
 for(let i=0;i<text.length;i++){let c=text[i],n=text[i+1];if(c==='"'&&quoted&&n==='"'){cell+='"';i++;continue}if(c==='"'){quoted=!quoted;continue}if(c===','&&!quoted){row.push(cell);cell='';continue}if((c==='\n'||c==='\r')&&!quoted){if(c==='\r'&&n==='\n')i++;row.push(cell);cell='';if(row.some(x=>x!==''))rows.push(row);row=[];continue}cell+=c}
 if(cell||row.length){row.push(cell);rows.push(row)};if(!rows.length)return[];let h=rows[0];return rows.slice(1).map(r=>{let o={};h.forEach((k,i)=>o[k]=r[i]??'');return o})
}
function pick(o,n){for(const k of n)if(o[k]!==undefined)return o[k];return''}
function money(v){v=+v||0;if(v>=1e9)return(v/1e9).toFixed(1)+' Md€';if(v>=1e6)return(v/1e6).toFixed(1)+' M€';if(v>=1e3)return Math.round(v/1e3)+' k€';return'—'}
function ageAt(dob){if(!dob)return99;let d=new Date(dob),t=new Date('2025-07-01');let a=t.getFullYear()-d.getFullYear();if(t.getMonth()<d.getMonth()||(t.getMonth()===d.getMonth()&&t.getDate()<d.getDate()))a--;return a}
function fmRating(p,v){let age=ageAt(p.date_of_birth),base=Math.log10((+v||0)+1)*10;return Math.min(99,Math.round(40+base+(age<21?8:age<24?5:age<28?2:0)))}
function closestValues(){
 let map=new Map();
 for(const v of realVals){let id=pick(v,['player_id']),d=new Date(pick(v,['date','valuation_date']));if(!id||isNaN(d)||d>new Date('2025-07-01'))continue;let old=map.get(id);if(!old||d>new Date(pick(old,['date','valuation_date'])))map.set(id,v)}
 return map
}
async function loadRealDB(){
 const s=document.getElementById('dbStatus');s.innerHTML='<b>⏳ Chargement...</b><br>La base réelle est volumineuse. La première ouverture peut prendre un peu de temps.';
 try{
  const [a,b]=await Promise.all([fetch(REAL_BASE+'players.csv'),fetch(REAL_BASE+'player_valuations.csv')]);
  if(!a.ok||!b.ok)throw Error('fetch');
  const [pt,vt]=await Promise.all([a.text(),b.text()]);realPlayers=csvParse(pt);realVals=csvParse(vt);
  const vm=closestValues();
  realPlayers=realPlayers.map(p=>{let v=vm.get(pick(p,['player_id']));let val=v?pick(v,['market_value_in_eur']):pick(p,['market_value_in_eur']);return {...p,_value:+val||0,_age:ageAt(p.date_of_birth),_rating:fmRating(p,val)}}).filter(p=>REAL_LEAGUES.has(pick(p,['current_club_domestic_competition_id']))&&p._age<60);
  realLoaded=true;document.getElementById('realFilters').style.display='block';s.innerHTML=`<b>✅ Base réelle chargée</b><br>${realPlayers.length.toLocaleString('fr-FR')} profils correspondant aux 5 championnats.<br><small>Valeur retenue : dernière valorisation connue au plus tard le 01/07/2025.</small>`;renderRealPlayers()
 }catch(e){s.innerHTML='<b>❌ Chargement impossible</b><br>Réessaie avec une bonne connexion. Aucun fichier FM26 ni donnée personnelle n’est envoyé.'}
}
function renderRealPlayers(){
 if(!realLoaded)return;let q=(realSearch.value||'').toLowerCase(),lg=realLeague.value,po=realPos.value,pm=+realPot.value||0,am=+realAge.value||99,vm=(+realValue.value||999)*1e6,so=realSort.value;
 let list=realPlayers.filter(p=>(!q||pick(p,['name']).toLowerCase().includes(q))&&(lg==='all'||pick(p,['current_club_domestic_competition_id'])===lg)&&(po==='all'||pick(p,['position'])===po)&&p._age<=am&&p._value<=vm&&p._rating>=pm).sort((a,b)=>so==='value'?b._value-a._value:so==='rating'?b._rating-a._rating:a._age-b._age);
 document.getElementById('realPlayers').innerHTML=list.slice(0,100).map(p=>`<div class="player"><div><b>${pick(p,['name'])}</b><br><small>${pick(p,['position'])} · ${p._age} ans · ${pick(p,['country_of_citizenship'])||'—'}</small><br><small>${money(p._value)} · pied ${pick(p,['foot'])||'—'}</small></div><div class="rating">${p._rating}</div></div>`).join('')||'<div class="result">Aucun joueur ne correspond.</div>'
}
