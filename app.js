...
async function searchRealDB(){
 ...
 status.innerHTML='<b>⏳ Recherche...</b><br>Le serveur prépare les résultats.';
 const p=new URLSearchParams({
   q:document.getElementById('realSearch').value||'',
   league:document.getElementById('realLeague').value,
   pos:document.getElementById('realPos').value,
   age:document.getElementById('realAge').value||35,
   value:document.getElementById('realValue').value||250
 });
 try{
   const r=await fetch('/api/players?'+p.toString());
   const d=await r.json();
   if(!r.ok) throw Error(d.error||'Erreur API');
   status.innerHTML=`<b>✅ Base disponible</b><br>${d.count} résultat(s) · référence 01/07/2025 · source ${d.source||'publique'}.`;
   renderScoutPlayers(d.players||[]);
 }catch(e){
   status.innerHTML='<b>❌ API indisponible</b><br>'+e.message;
   box.innerHTML='';
 }
}
...
function renderScoutPlayers(list){
  window.lastScoutingResults = list || [];
  lastScoutingResults = window.lastScoutingResults;
  const box=document.getElementById('realPlayers');
...
function openPlayerModal(i){
  const p=(window.lastScoutingResults||lastScoutingResults)[i];
  if(!p)return;

  const modal=document.getElementById('playerModal');
  const content=document.getElementById('modalContent');
  if(!modal||!content){
    // Fallback if an old cached HTML is still served.
    alert('Fiche joueur indisponible : actualise la page.');
    return;
  }

  const budget=clubBudget();
  const comp=compatibility(p);
  const valueM=(+p.value||0)/1e6;
  const affordability=valueM<=budget;
  const resale=p.age<=21?'Très élevé':p.age<=24?'Élevé':p.age<=27?'Moyen':'Faible';

  content.innerHTML=`
    <div class="modal-kicker">FICHE SCOUTING FM26+</div>
    <h2>${escapeHtml(p.name)}</h2>
    <div class="modal-rating">${p.rating}<span>/99</span></div>
    <div class="modal-grid">
      <div><small>POSTE</small><b>${escapeHtml(p.position||'—')}</b></div>
      <div><small>ÂGE</small><b>${p.age} ans</b></div>
      <div><small>NATIONALITÉ</small><b>${escapeHtml(p.nationality||'—')}</b></div>
      <div><small>PIED</small><b>${escapeHtml(p.foot||'—')}</b></div>
      <div><small>VALEUR</small><b>${formatMoney(p.value)}</b></div>
      <div><small>COMPATIBILITÉ</small><b>${comp}%</b></div>
    </div>
    <div class="modal-section">
      <h3>🎯 Analyse pour ton club</h3>
      <div class="analysis-line">${affordability?'🟢':'🔴'} ${affordability?'Valeur dans le budget de référence':'Valeur supérieure au budget de référence'} (${budget.toFixed(1)} M€)</div>
      <div class="analysis-line">💎 Potentiel de revente : <b>${resale}</b></div>
      <div class="analysis-line">${p.age<=23?'🟢 Jeune profil intéressant pour progression/revente.':'🟡 Profil plus expérimenté.'}</div>
    </div>
    <div class="modal-actions">
      <button class="primary" id="shortlistBtn">♡ Ajouter à la shortlist</button>
      <button id="offerBtn">💶 Préparer une offre</button>
    </div>`;
  ...
  if(close) close.onclick=closePlayerModal;
  const shortlist=document.getElementById('shortlistBtn');
  if(shortlist) shortlist.onclick=()=>addShortlist(p);
  const offer=document.getElementById('offerBtn');
  if(offer) offer.onclick=()=>prepareOffer(p);
}
...
function addShortlist(p){
  const list=JSON.parse(localStorage.getItem('fm26shortlist')||'[]');
  if(!list.some(x=>String(x.id)===String(p.id))) list.push(p);
  localStorage.setItem('fm26shortlist',JSON.stringify(list));
  const b=document.getElementById('shortlistBtn');
  if(b){b.textContent='✓ Ajouté à la shortlist';b.disabled=true;}
}

function prepareOffer(p){
  const budget=clubBudget();
  const valueM=(+p.value||0)/1e6;
  const recommended=Math.max(0,Math.min(budget*0.9,valueM*0.85));
  const b=document.getElementById('offerBtn');
  if(b)b.textContent=`Offre conseillée : ${recommended.toFixed(1)} M€`;
}
